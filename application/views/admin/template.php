...
<?php
function tampilkanMenuBertingkat ($menu,$in) {
	if(isset($menu[$in]) && $menu[$in] != NULL){
	echo "<ul class='tree '>";
	foreach ($menu[$in] as $key => $item) {?>
		<li class="<?=$item['permalink'];?> mf"> <a  style="cursor: pointer" onclick="load_url('#cont', '<?=site_url('prodi/folder/'. $item['permalink'])?>','<?=$item['permalink'];?>')">
	  ...
	  <?php if (@$menu[$item['id_folder']] && count($menu[$item['id_folder']])) {
		tampilkanMenuBertingkat($menu,$item['id_folder']);
	  }
	}
	echo "</ul>";
	}
	...
	<div class="navbar navbar-default header-highlight">
		...
		<div class="navbar-collapse collapse" id="navbar-mobile">
			...
			<div class="navbar-right">
				<p class="navbar-text">Hello, <?= $this->session->userdata('name'); ?></p>
				<ul class="nav navbar-nav">
					...
					<div class="sidebar-category sidebar-category-visible">
						<div class="category-content no-padding" style="overflow: auto;">
							<ul class="navigation navigation-main navigation-accordion">
								...
								<li class="rps hide"><a href="<?=site_url('prodi/rps');?>" class="legitRipple"><i class="icon-book"></i> <span>RPS</span></a></li>
								<li class="folder"><a href="<?=site_url('prodi/folder?f');?>" class="legitRipple"><i class="icon-folder"></i> <span>Dokumen</span></a></li>
								<?php if(isset($folder)) tampilkanMenuBertingkat($folder,0) ?>
							</ul>
						</div>
					</div>
					<!-- /main navigation -->
